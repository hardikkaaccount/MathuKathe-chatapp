...
async function testTwin() {
    ...
    try {
        ...
        const user = await Promise.resolve().then(() => __importStar(require("../models/user"))).then(m => m.User.create({ email, display_name: "TwinTester", password_hash: "hash" }));
        if (!user || !user.id) {
        ...
        `;
        const groupResult = await (0, hasura_1.execute)(INSERT_GROUP_MUTATION, { name: "Twin Test Group", created_by: safeUserId });
        ...
        `;
        await (0, hasura_1.execute)(INSERT_MESSAGE_MUTATION, { content: "Hey, do you want to go grab lunch?", group_id: groupId, sender_id: safeUserId });
        await (0, hasura_1.execute)(INSERT_MESSAGE_MUTATION, { content: "I was thinking about pizza.", group_id: groupId, sender_id: safeUserId });
        // 3. Trigger AI Twin
        ...
        `;
        const result = await (0, hasura_1.execute)(TWIN_REPLY_MUTATION, { group_id: groupId });
...
//# sourceMappingURL=testTwin.js.map